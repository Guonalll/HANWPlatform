...
public class CountryImpl implements CompInterface {
	...
	private static void generateFarmerInfos(int simulationId) {
		...
		List<String[]> readExcel = Jexcel.readExcel("E:/Guonalll/2ABM模型/3HANWMPlatform-master/resource/import/importdatabefore1.xlsx", 0);
//		List<String[]> readExcel = Jexcel.readExcel("E:/Guonalll/2ABM模型/3HANWMPlatform-master/resource/import/importdataafter.xlsx", 0);
...
//				Double sense = Generator.Normal(Double.valueOf(simulation.get("sense").toString()), cv);
				...
				record.put("crop_area", readExcel.get(i)[1]);
//				比率
				record.put("mu", readExcel.get(i)[2]);
				record.put("learn", readExcel.get(i)[3]);
				record.put("radius", readExcel.get(i)[4]);
				record.put("sense", readExcel.get(i)[5]);
